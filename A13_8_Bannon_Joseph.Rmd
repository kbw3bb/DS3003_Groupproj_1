...
<!--   scale_fill_gradient(low="blue", high="red", name='Energy Consumption per capita \n(billion BTUs / person)')+ labs(x = 'Longitude', y = 'Latitude', title="Energy Consumption per Capita in the US by state") -->
<!-- ggplotly(g) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- US_energy_map_mod$hover <- with(US_energy_map_mod, paste(NAME, '<br>', "Energy:", format(round(Consumption_per_capita_2010, 2), 2))) -->
...
<!--   ) -->
<!--  plot_ly(US_energy_map_mod, z = ~Consumption_per_capita_2010, text = ~hover, locations = ~STUSPS,  -->
<!--         type = 'choropleth', locationmode = 'USA-states',  -->
<!--         color = ~Consumption_per_capita_2010,  -->
<!--         colors = 'Reds',  -->
<!--         marker = list(line = borders)) %>% -->
<!--      layout(title = 'Energy Consumption per Capita in the US by state', geo = map_options) %>%  -->
<!--      colorbar(title = "Energy Consumption per capita \n(billion BTUs / person)") -->
<!-- ``` -->
<!-- --> 
